...
function calc_taxes(taxRates, income, itemized_deductions, exemptions, w) {
    ...
    if (income <= taxRates.new_york_state.federal_itemized_deductions_agi_limit[cat])
    ...
    if (income > taxRates.federal.personal_exemption_phaseout_maximum_agi[cat])
    ...
    social_security_taxable_income = Math.min(income, taxRates.federal.social_security_taxable_income_limit[cat]);
